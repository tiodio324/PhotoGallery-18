...
export const LoginModal = observer(() => {
  ...
  const [selectedRole, setSelectedRole] = useState<LoginRole>('photographer');
  ...
  const handleClose = () => { closeLoginModal(); setPassword(''); setSelectedRole('photographer'); };

  return (
    <Modal isOpen={loginModalOpen} onClose={handleClose} title="Вход в систему" size="sm">
      <form onSubmit={handleSubmit} className={styles.form}>
        <div className={styles.roleSelector}>
          <button type="button" className={`${styles.roleButton} ${selectedRole === 'photographer' ? styles.active : ''}`} onClick={() => setSelectedRole('photographer')}>
